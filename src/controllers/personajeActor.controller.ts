...
export const associateActorToPersonaje = async (
  req: Request,
  res: Response
) => {
  ...
  try {
    ...
    if (
      ...
      actorId.some((id) => isNaN(id))
    ) {
      return res
        .status(400)
        .json({
          message:
            "ID de personaje debe ser un número válido y actorId debe ser un array de números",
        });
    }
    ...
    if (actors.length !== actorId.length) {
      ...
      return res
        .status(404)
        .json({ message: "Uno o más actores no encontrados" });
    }
    ...
    res
      .status(200)
      .json({ message: "Actores asociados al personaje con éxito" });
  } catch (error) {
...
export const desasociarActorDePersonaje = async (
  req: Request,
  res: Response
) => {
  ...
  try {
    ...
    if (
      !personajeId ||
      isNaN(Number(personajeId)) ||
      !actorId ||
      isNaN(Number(actorId))
    ) {
    ...
    if (resultado) {
      return res
        .status(200)
        .json({ message: "Asociación eliminada con éxito" });
    } else {
...
export const getActoresAsociadosConPersonaje = async (
  req: Request,
  res: Response
) => {
  ...
  try {
    ...
    if (actores.length > 0) {
      ...
      return res
        .status(404)
        .json({ message: "No se encontraron actores asociados" });
    }
  } catch (error) {
